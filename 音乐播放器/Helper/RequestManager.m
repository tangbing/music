...
//         NSLog(@"%@",dataPathString);
        ...
        if (!error) {
            ...
            for (NSDictionary *dic in array) {
                ...
                //歌单播放次数(NSNumber类型)
                music.playCount = dict[@"playCount"];
                //订阅数
                music.subscribedCount =[NSString stringWithFormat:@"%@",dict[@"subscribedCount"]];
                //歌单评论数
                music.commentCount = [NSString stringWithFormat:@"%@",dict[@"commentCount"]];
                //歌单分享数
                music.shareCount =[NSString stringWithFormat:@"%@",dict[@"shareCount"]];
                //歌单内歌曲个数
...
-(NSError *)networkInformation{
    ...
    //设置网络请求
    NSURLRequest *request = [[NSURLRequest alloc]initWithURL:[NSURL URLWithString:kMusicList] cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:0];
